...
## 📌 從 01 升級到 02（做過 01 的人看這裡）

> 沒做過 01？跳過本節，直接到「上課目標」。

### 01 你做了什麼 → 02 升級了什麼

| 01 的狀態 | 02 的變化 | 為什麼 |
|-----------|----------|--------|
| 一個 Bot，什麼都自己做 | 5 個 Agent 各司其職 | 並行 + 故障隔離 |
| `planner.py` if-else 路由 | leader 用 TaskGraph 拆任務 | 能力越多越複雜 → 需要拆人 |
| 一個 Gemini session | 每人獨立 kiro-cli | 互不干擾，各有記憶 |
| 手動加 Skill + 重啟 | 加 Agent，不需重啟 | 熱擴展 |
| 科技日報一人做 | market 爬 + report 渲染 | 30 秒並行完成 |
...
01 你學的               02 在哪裡用
─────────────────       ─────────────────
意圖路由概念        →   每個 Agent 內部都有自己的 Planner
BaseSkill 架構      →   每個 Agent 有獨立 Skills 目錄
Gemini 對話        →   Agent 後端用 kiro-cli（更強大版）
排程系統           →   scheduler.yaml 可排程到不同 Agent
科技日報流程        →   market + report 分工版
```
