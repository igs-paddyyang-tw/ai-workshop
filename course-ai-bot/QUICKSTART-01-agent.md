...
**做什麼**：為遊戲公司實際業務設計 SOUL  
**為什麼**：這才是帶走的核心能力 — 回去馬上能用

📝 Kiro IDE 輸入（選一個場景）：

**場景 A：遊戲企劃助手**
```
設計一個「遊戲企劃助手」的 SOUL.md：
- 身份：資深遊戲設計師，10 年手遊經驗
- 擅長：玩法設計、數值平衡、Boss 機制
...
```
用遊戲企劃助手的人格回答：
「設計一個新 Boss，要讓玩家覺得刺激但不會太難」
```

**場景 B：營運數據分析師**
```
設計一個「營運分析師」的 SOUL.md：
- 身份：數據分析專家，熟悉手遊 KPI
- 擅長：DAU/MAU、付費率、ARPU、留存率
- 格式：數據 → 趨勢 → 異常 → 建議
- 語氣：用數字說話，不猜測
```
...
2. `/agents` → QA → 問「爆率 1:500 合理嗎？」
   - ✅ 預期：❌/⚠️/✅ 標記（遊戲測試員）
3. `/agents` → Admin（替換為企劃助手或分析師）→ 問業務問題
   - ✅ 預期：完全符合你設計的場景
