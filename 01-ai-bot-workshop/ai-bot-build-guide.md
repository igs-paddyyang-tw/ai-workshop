...
| 維度 | 01 單兵模式 | 02 Agent Team |
|------|------------|---------------|
| 並行能力 | ❌ 序列（一次一個） | ✅ 5 Agent 並行 |
| 故障隔離 | ❌ 一掛全掛 | ✅ 一個掛不影響其他 |
| 擴充方式 | 改 3 檔 + 重啟 | 加 Agent + 不重啟 |
| 科技日報 | 一人做到死 | market 爬、report 渲染、分工明確 |
| 知識累積 | 沒有 | 每個 Agent 有獨立知識庫 |
...
> 它們會在 02 的團隊裡被重新定位，成為 market-agent 或 coder-agent 的「個人能力」。
> 
> 🔜 **下堂課（02）**：5 分鐘建好團隊 → 一句話派工 → 10 秒並行完成你的 3 個需求。
